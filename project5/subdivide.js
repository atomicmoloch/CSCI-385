...
class Surface {
    ...
    subdivide() {
        ...
        for (let v of S.allVertices()) {
            //even vertex rule
            //applies general averaging rule to cloned vertices
            let k = 1;
            let summa = v.edge.target.position;
            let outgoing = v.edge.prev.twin;
            //iterates through outgoing edges in ccw fashion
            while (! (outgoing.target == v.edge.target)) {
                k++;
                summa = summa.plusPoint(outgoing.target.position);
                outgoing = outgoing.prev.twin;
              //  summa = summa.plusPoint(outgoing.target.position.times(1/k))
            }

            summa = summa.times(1/k);
            let beta = (5/8) - ((3/8) + ((1/4) * Math.cos(2 * Math.PI/k)))**2;

            v.clone = R.makeVertex(v.position.combo(beta, summa));
        }
        // Step 2
        for (let e of S.allEdges()) {
            if (e.twin.split) {
                e.split = e.twin.split;
                //whenever i try to take this line out I end up with invisible duplicate edges
            }
            else {
                //applies odd vertex rule
                e.split = R.makeVertex(
                    (e.source.position).combo(0.5, e.target.position)
                    .combo(0.25,
                    (e.twin.prev.source.position.combo(0.5, e.next.target.position))));
            }
        }
        ...
        for (let f of S.allFaces()) {
           // const v0 = f.edge.target.clone.id;
            const v0 = f.edge.source.clone.id;
            const v01 = f.edge.split.id;
          //  const v1 = f.edge.next.target.clone.id;
            const v1 = f.edge.next.source.clone.id;
            const v12 = f.edge.next.split.id;
          //  const v2 = f.edge.prev.target.clone.id;
            const v2 = f.edge.prev.source.id;
            const v20 = f.edge.prev.split.id;


            //not yet making sure order is right
            R.makeFace(v01, v12, v20);
            R.makeFace(v20, v0, v01);
            R.makeFace(v01, v1, v12);
            R.makeFace(v12, v2, v20);

      //      R.makeFace(v0, v01, v20);


         //   R.makeFace(v0, v20, v01);
        //    R.makeFace(v0, v01, v20);
        //    R.makeFace(v01, v0, v20);
        //    R.makeFace(v01, v20, v0);
        //    R.makeFace(v20, v0, v01);
        //    R.makeFace(v20, v01, v0);
