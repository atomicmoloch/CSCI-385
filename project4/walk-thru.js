...
class Shot {
    constructor(position0, direction0) {
        this.position = position0; //Point3D
        this.direction = direction0; //Vector3D
    }

    combo(amount, other) {
        const posCombo = this.position.combo(amount, other.position);
        const dirCombo = this.direction.combo2d(amount, other.direction).unit();
      //  console.log(dirCombo);
        return new Shot(posCombo, dirCombo);
    }
}
...
//Implements Chaikin subdivision on a list of shots
function smoothPath(shotList) {
    let smoothList = [shotList[0]]
    for (let index = 1; index < shotList.length; index++) {
        smoothList.push(shotList[index-1].combo(0.25, shotList[index]));
        smoothList.push(shotList[index-1].combo(0.75, shotList[index]));
    }
    smoothList.push(shotList[shotList.length - 1]);
    return smoothList
}
...
class WalkThru {
    ...
    toPDF(document, startNewPage) {
        ...
        //
        const cameras = [];
        const smoothshots = smoothPath(smoothPath(this.shots));

        for (let shot of smoothshots) {
            // Performs STEP 1.
...
class SceneCamera {
    ...
    constructor(center, towards, upward) {
        ...
        this.center = center;
        this.right  = (towards.unit()).cross(upward.unit());
        this.up     = this.right.cross(towards.unit());
    ...
    project(location) {
        ...
        //Calculates z coord distance (depth) between point and viewpoint, and scales coordinate respectively
        const distance = location.minus(this.center);
        const zDistance = this.into.dot(distance);
        const projPtVec = (distance.div(zDistance));

        //point: the given point
        //projection: x and y coordinates of the projected point
        //depth: z coord distance between viewpoint and point
        const result = {
            point: location,
            projection: new Point2d(projPtVec.dot(this.right), projPtVec.dot(this.up)),
            depth: zDistance
        };
...
class SceneEdge {
    ...
    breakpoints(edges) {
        ...
        var crossings = [0.0, 1.0];
        for (let e of edges) {
            var intersection = segmentsIntersect(this.start.projection, this.end.projection, e.start.projection, e.end.projection);
            if ( (!(intersection == null)) && ((intersection > EPSILON) && (intersection < 1 - EPSILON)) ) {
                crossings.push(intersection);
            }
        }
     //   crossings.filter((crossing) => !(crossing == null));
        crossings.sort(function(a, b){return a-b}); //sorts breakpoints in ascending order
        return crossings;
    }

    isSegmentVisible(breakpointA, breakpointB, camera, objects) {
        var Q1, Q2, Q3;
        //line ray end set to center of edge
        //ray start set to camera.center
        const lineMiddlePoint = breakpointA.plus( (breakpointB.minus(breakpointA)).times(1.0/2.0) );
        const cameraPoint = camera.center;

        for (let o of objects) {
            for (let f of o.faces) {
                Q1 = f.vertex(0).position;
                Q2 = f.vertex(1).position;
                Q3 = f.vertex(2).position;

                const rIntercept = rayFacetIntersect(Q1, Q2, Q3, cameraPoint, lineMiddlePoint);
                if (rIntercept) {
                    if (!(this.faces.includes(f)) && (rIntercept.distance < lineMiddlePoint.dist(cameraPoint))) { //discounts interception if intercepting face is further from camera or a bordering facet to the edge
                    //possibly unnecessary code but better safe than sorry for edge cases
                        return false;
                    }
                }
            }
    ...
    draw(document, camera, segments, objects) {
        ...
        const pVector =  (this.end.point).minus(p0);
        ...
        for (let c = 0; c < crossings.length - 1; c++) {
            var breakpointA = p0.plus(pVector.times(crossings[c]));
            var breakpointB = p0.plus(pVector.times(crossings[c+1]));
            if (this.isSegmentVisible(breakpointA, breakpointB, camera, objects)) {
            //    console.log((camera.project(breakpointA)).projection);
                const ppdf0 = toPDFcoord((camera.project(breakpointA)).projection);
                const ppdf1 = toPDFcoord((camera.project(breakpointB)).projection);
             ...
             //   document.circle(ppdf1.x, ppdf1.y, 0.35, "F"); //these get pretty ugly with breakpoints
            }
        }
