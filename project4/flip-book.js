...
function drawCameraPath() {
     ...
     */
    const smoothList = smoothPath(smoothPath(gWalkThru.shots));
    //const smoothList = (smoothPath(gWalkThru.shots));
    const cc = CAMERA_COLOR;
    const occ = OFF_CAMERA_COLOR;
    const pc = WALKTHRU_COLOR;
    for (let index = 1; index < smoothList.length; index++) {
        const shot0 = smoothList[index-1];
        const shot1 = smoothList[index];
        ...
        //shews direction of intermediate shots at either end of the drawn line
        const direction0 = shot0.direction;
        const angle0 = Math.atan2(direction0.dy, direction0.dx) * 180.0 / Math.PI;
        glPushMatrix();
        glTranslatef(position0.x, position0.y, position0.z);
        ...
        const direction1 = shot1.direction;
        const angle1 = Math.atan2(direction1.dy, direction1.dx) * 180.0 / Math.PI;
        glPushMatrix();
...
async function makePDF() {
    ...
    doc.text("by Kavi Wilson", 29, 37, null, null, "center");
