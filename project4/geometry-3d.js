...
class Vector3d {
    ...
    rotate2d(deg) {
        ...
        return new Vector3d((Math.cos(angle) * this.dx) - (Math.sin(angle) * this.dy), (Math.sin(angle) * this.dx) + (Math.cos(angle) * this.dy), 0.0);
    }

    //returns a linear interpolation of the directions of this and other
    combo2d(amount, other) {
        const angle0 = Math.atan2(this.dy, this.dx) * 180.0 / Math.PI;
    //    const angle1 = Math.atan2(other.dy, other.dx) * 180.0 / Math.PI;
        //Determines difference in angle of the two vectors
        const anglediff = -Math.atan2(this.dy * other.dx - this.dx * other.dy, this.dx * other.dx + this.dy * other.dy) * 180.0 / Math.PI;
        const angleLerp = angle0 + (amount * (anglediff));
       // console.log(angleLerp);
        return (new Vector3d(1.0, 0.0, 0.0)).rotate2d(angleLerp);
    }
}
